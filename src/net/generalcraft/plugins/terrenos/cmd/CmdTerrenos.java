package net.generalcraft.plugins.terrenos.cmd;
...
public class CmdTerrenos implements CommandExecutor {
	...
	public CmdTerrenos(GeneralcraftAPI main) {
		plugin = main;
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		case "ver":
			...
			if (args.length == 1) {
				...
				try {
					...
					new SysTerrenosVerBlocos(player, plugin, length);
		...
		case "comprar":
			...
			try {
				...
				new SysTerrenosComprar(plugin, player, length);
			} catch (NumberFormatException e) {
				sender.sendMessage(prefix + messages.getString("ValorNumerico"));
			}
			break;
