package net.generalcraft.plugins.terrenos.system;
...
public class SysTerrenosComprar {
	...
	public SysTerrenosComprar(GeneralcraftAPI plugin, Player player, Integer length) {
		this.plugin = plugin;
		this.player = player;
		this.length = length;
	...
	private void run() {
		if (length <= 10) {
			if (!SysEconomy.hasMoney(player, 100.0)) {
			...
			if (!SysEconomy.hasMoney(player, 500.0)) {
				player.sendMessage(prefix + messages.getString("CoinInsuficiente"));
				return;
			}
		} else {
			if (!SysEconomy.hasMoney(player, 1000.0)) {
				player.sendMessage(prefix + messages.getString("CoinInsuficiente"));
				return;
			}
		}
		...
		if (!new SysTerrenosVerify(plugin, player, length).run()) {
		...
		new SysTerrenosSetBlock(player, plugin, length);
